...
`)
	if err != nil {
		httpx.ErrorJSON(w, "query backup_tasks fail", err)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &enableInt, &lastUpdate, &lastBackup, &nextBackup, &cfgJSON); err != nil {
			httpx.ErrorJSON(w, "scan backup_tasks fail", err)
			return
		}
		...
		item := map[string]interface{}{
			...
			"lastBackupTime":     httpx.ConvertTimeToJST(lastBackup),
			"nextBackupTime":     httpx.ConvertTimeToJST(nextBackup),
		}
	...
	if err := rows.Err(); err != nil {
		httpx.ErrorJSON(w, "backup_tasks iteration error", err)
		return
	}
...
func BackupRunHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpx.ErrorJSON(w, "query fail", err)
		return
	}
...
func BackupUpdateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		httpx.ErrorJSON(w, "fetch existing config fail", err)
		return
	}
...
func getDbAndCronManager() (*sql.DB, *CronManager) {
	...
	return db, NewCronManager(db, apiServer)
}
