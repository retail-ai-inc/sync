...
func TestBackupControlIsReachableWithoutCredentials(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			useTempTaskDB(t)
			// The backup package's in-memory task-status registry is not reset
			// here: it is unexported, and a stale entry cannot change whether an
			// endpoint answers without credentials, which is all this records.
			unauthenticated(t, tc.name, tc.handler, tc.req, tc.params)
		})
	}
}
...
func TestConnectionProbeIsReachableWithoutCredentials(t *testing.T) {
	rec := unauthenticated(t, "POST /api/test-connection", dbinspect.TestConnectionHandler,
...
func TestSchemaReadIsReachableWithoutCredentials(t *testing.T) {
	unauthenticated(t, "POST /api/tables/schema", dbinspect.GetTableSchemaHandler,
