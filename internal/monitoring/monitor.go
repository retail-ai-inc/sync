...
func SendTableComparisonSlackNotification(ctx context.Context, sc config.SyncConfig, srcDB, srcTable, tgtDB, tgtTable string,
	...
	slackNotifier := notify.NewSlackNotifierFromConfig(cfg, log)
	...
	status := "✅"
	alertType := notify.SlackAlertGood

	if difference != 0 {
		status = "⚠️"
		alertType = notify.SlackAlertWarning
	}
	...
	go func() {
		...
		err := slackNotifier.SendNotification(ctx, message, &notify.SlackNotificationOptions{
			AlertType: alertType,
